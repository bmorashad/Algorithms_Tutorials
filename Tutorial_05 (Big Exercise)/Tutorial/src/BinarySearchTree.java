...
public class BinarySearchTree {
     ...
     */
    private void output(int[] arr, int start, int end) {
        if (start > end) return;

        // print left subtree
        output(arr, start*2 + 1, end);

        // print root
        System.out.print(arr[start] + " ");

        // print right subtree
        output(arr, start*2 + 2, end);
    }
    ...
    */
    private void insertBelow(TreeNode node, int value) {
        ...
        */
        if (value <= root.data) {
            insertBelow(root.leftChild, value);
        } else {
            insertBelow(root.rightChild, value);
        }
    }
    ...
    private void removeItem(TreeNode root, int value) {

        if (value <= root.data) {
            removeItem(root.leftChild, value);
        } else if (value > root.data) {
            removeItem(root.rightChild, value);
        } else {
            if (root.leftChild != null && root.rightChild != null) {
                ...
                removeItem(root.rightChild, root.data);
            }
        }

    }

    private int minValue(TreeNode rightChild) {
        int minv = root.data;
    ...
    public void insertAll(int[] values, boolean printResults) {
        for (int i = 0; i < values.length; i++) {
            insert(values[i]);
            if (printResults)
                output(values, 0, values.length - 1);
        }
    }
